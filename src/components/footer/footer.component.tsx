...
const IconButton = ({icon, text, isActive, onClick}: {
    ...
    return (
        <li onClick={onClick} className={isActive ? `${classes} primary-color-bold` : classes}>
        ...
        <div>
            <ul className="list">
                {buttons.map(({icon, text, isActive}) => (
                    <IconButton key={text} icon={icon} text={text} isActive={isActive}
                                onClick={() => changeView(text)}></IconButton>
                ))}
            </ul>
        </div>
    );
