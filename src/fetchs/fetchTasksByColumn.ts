...
export const fetchTasksByColumn = async ({queryKey}: QueryFunctionContext<QueryKey, any>): Promise<{
    columnId: string;
    tasks: Task[]
}> => {
    const [, columnId] = queryKey;
    return {tasks: createTasksMocks(columnId as string, 10), columnId: columnId as string};
};