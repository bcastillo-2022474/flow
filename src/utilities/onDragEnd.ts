...
const onDragEnd = (
    ...
    setColumnsTasks: SetColumnsTasks
) => {
    if (!result.destination) return;

    const itemsDestination = columnsTasks.find(
        ({ columnId }) => columnId === result.destination?.droppableId
    )?.tasks;
    const itemsSource = columnsTasks.find(
        ({ columnId }) => columnId === result.source.droppableId
    )?.tasks;
